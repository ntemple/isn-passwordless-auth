 ...
 *
 */
define( 'PASSWORDLESS_LOGIN_VERSION', '1.1.1' );
...
function wpa_basic_info_content() {
    ...
    <div class="wrap wpa-wrap wpa-info-wrap">
        ...
        <p>
            <strong style="font-size: 30px; vertical-align: middle; color:#d54e21;"><?php echo get_option( 'wpa_total_logins', '0' ); ?></strong> <?php _e( 'successful logins without passwords.', 'passwordless-login' ); ?>
        </p>
        <hr/>
        ...
        <div>
            ...
            <div class="wpa-row wpa-3-col">
                <div>
                    <p><?php _e( 'Front-End registration, edit profile and login forms.', 'passwordless-login' ); ?></p>
                </div>
                <div>
                    <p><?php _e( 'Drag and drop to reorder / remove default user profile fields.', 'passwordless-login' ); ?></p>
                </div>
                <div><p><?php _e( 'Allow users to log in with their username or email.', 'passwordless-login' ); ?></p>
                </div>
                <div>
                    <p><?php _e( 'Enforce minimum password length and minimum password strength.', 'passwordless-login' ); ?></p>
                </div>
            </div>
            <p><a href="https://wordpress.org/plugins/profile-builder/"
                  class="button button-primary button-large"><?php _e( 'Learn More About Profile Builder', 'passwordless-login' ); ?></a>
            </p>
        </div>
    </div>
	<?php
	...
	if ( $account && ! is_wp_error( $sent_link ) ) {
        ...
        <form name="wpaloginform" id="wpaloginform" action="" method="post">
            <p>
                ...
                <input type="text" name="user_email_username" id="user_email_username" class="input"
                       value="<?php echo esc_attr( $account ); ?>" size="25"/>
                <input type="submit" name="wpa-submit" id="wpa-submit" class="button-primary"
                       value="<?php esc_attr_e( 'Log In', 'passwordless-login' ); ?>"/>
            </p>
